# LocAgence Pro — Download Page

Page de téléchargement publique pour **LocAgence Pro** (Windows).

## Fichiers

- `index.html` — page de téléchargement
- `style.css` — styles
- `LocAgencePro-Setup-1.0.5.exe` — installateur Windows (v1.0.5, fix Windows 11)
- `CLE-ESSAI-7-JOURS.txt` — clé d'essai gratuite 7 jours

## GitHub Pages

Pour activer le site :
1. Repo → **Settings** → **Pages**
2. Source : **Deploy from branch** → `main` → `/ (root)`
3. URL : `https://chouibak.github.io/Desktop-app-rental-car-exe/`
